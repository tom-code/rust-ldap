...
struct Context {
    ...
    notif: tokio::sync::oneshot::Sender<Vec<Message>>,
}
...
impl ClientConnection {
    async fn send_raw(&self, data: Vec<u8>) -> Result<()> {
        let res = self.req_writer.send(data).await;
        match res {
            Ok(_) => Ok(()),
    ...
    async fn send_request(&self, msg: ldap::Message) -> Result<()> {
        let tosend = msg.encode()?;
        self.send_raw(tosend).await
    }
    ...
    pub async fn send_request_wpar(&self, par: ldap::MessageParams) -> Result<Vec<Message>> {
        let msg = Message {
            id: self
                .last_id
        ...
        self.send_request_w(msg).await
    }

    pub async fn send_request_bind(&self, name: &str, password: &str) -> Result<MsgBindResponse> {
        let params = MessageParams::Bind(MsgBind {
            version: 3,
            name: name.to_owned(),
            password: password.to_owned(),
        });

        let mut res = self.send_request_wpar(params).await?;
        if res.len() == 1 {
