...
pub fn ldap_read_filter_attr_val_assertion(
    cursor: &mut Cursor<&[u8]>,
) -> Result<FilterAttributeValueAssertion> {
    ...
    Ok(FilterAttributeValueAssertion { name, value })
}
...
pub fn ldap_read_filter(cursor: &mut Cursor<&[u8]>) -> Result<Filter> {
    ...
    match tag {
        ...
        0xa3 => {
            // equality match
            Ok(Filter::EqualityMatch(ldap_read_filter_attr_val_assertion(
                cursor,
            )?))
        }
        0x87 => {
...
fn enc_filter(e: &mut asn1::Encoder, f: &Filter) -> Result<()> {
    match f {
        ...
        Filter::Present(f) => e.write_octet_string_with_tag(0x87, f.name.as_bytes()),
        Filter::And(f) => {
            e.start_seq(0xa0)?;
            for a in &f.items {
                enc_filter(e, a)?
...
pub fn ldap_write_search_res_entry(
    id: u32,
    name: &str,
    attrs: &Vec<crate::ldap::PartialAttribute>,
) -> Result<Vec<u8>> {
    let mut e = asn1::Encoder::new();
    e.start_seq(0x30)?;
    e.write_int(id)?;
    ...
    match msg_tag {
        ...
        0x64 => {
            ...
            while cursor.position() < (start_seq_len + 2) as u64 {
                ...
                let attribute = crate::ldap::PartialAttribute {
                    name: attr_name,
                    values: attr_values,
                };
            ...
            Ok((
                Message {
                    id: message_id,
                    params: MessageParams::SearchResult(MsgSearchResult {
                        name,
                        values: partial_attr_list,
                    }),
                },
                start_seq_len + 2,
            ))
        ...
        0x65 => {
            ...
            Ok((
                Message {
                    id: message_id,
                    params: MessageParams::MsgSearchResultDone(MsgSearchResultDone { res }),
                },
                start_seq_len + 2,
            ))
        }
...
impl Message {
    pub fn encode(&self) -> Result<Vec<u8>> {
        match &self.params {
            MessageParams::Bind(b) => ldap_write_bind_request(self.id, &b.name, &b.password),
            MessageParams::BindResponse(r) => ldap_write_bind_response(self.id, r.res),
            MessageParams::Search(s) => ldap_write_search_request(self.id, s),
            MessageParams::SearchResult(r) => {
                ldap_write_search_res_entry(self.id, &r.name, &r.values)
            }
            MessageParams::MsgSearchResultDone(d) => ldap_write_search_res_done(self.id, d.res),
            MessageParams::Unbind(_) => Err(std::io::Error::new(
...
#[test]
fn bind_test() {
    let d1 = parse_message(
        hex::decode("3013020101600e0201030402787880056865736c6f".as_bytes())
            .unwrap()
            .as_ref(),
    )
    .unwrap();
    ...
    assert_eq!(
        encoded,
        hex::decode("3013020101600e0201030402787880056865736c6f".as_bytes()).unwrap()
    );
}
